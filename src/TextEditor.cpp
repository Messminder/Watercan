...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		// Use ImGui::Shortcut() for modifier+key combos (proper routing in ImGui 1.90+)
		if (!IsReadOnly() && ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_Z))
			Undo();
		else if (!IsReadOnly() && !ctrl && !shift && alt && ImGui::IsKeyPressed(ImGuiKey_Backspace))
			Undo();
		else if (!IsReadOnly() && ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y))
			Redo();
			...
			mOverwrite ^= true;
		else if (ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert))
			Copy();
		else if (ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_C))
			Copy();
		else if (!IsReadOnly() && ImGui::Shortcut(ImGuiMod_Shift | ImGuiKey_Insert))
			Paste();
		else if (!IsReadOnly() && ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_V))
			Paste();
		else if (ImGui::Shortcut(ImGuiMod_Ctrl | ImGuiKey_X))
			Cut();
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::JSON()
{
	...
	if (!inited)
	{
		...
		// JSON strings (double-quoted with escape handling)
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String));
		// JSON numbers (integer, float, scientific notation, negative)
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("-?[0-9]+([.][0-9]+)?([eE][+-]?[0-9]+)?", PaletteIndex::Number));
		// Identifiers (for keyword matching: true, false, null)
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Identifier));
		// Punctuation: { } [ ] : ,
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[\\{\\}\\[\\]\\:\\,]", PaletteIndex::Punctuation));
