...
## Building an AppImage (recommended for a single-file Linux package)

A helper script is provided to create an AppImage that bundles the application into a single executable. The generated AppImage will be placed under the `Standalone/` directory.

Requirements:
- curl or wget
- FUSE (for running AppImages) or `--appimage-extract` can be used to inspect
...
# Then run the AppImage builder (produces Standalone/Watercan.AppImage)
cd ..
./scripts/build_appimage.sh build Standalone
```

Notes:
- The script attempts to download `linuxdeploy` and `appimagetool` automatically if they are not available on your PATH.
- The resulting AppImage will still depend on system graphics drivers and may require a desktop session to run; it is more portable than raw binaries but not guaranteed to run on every distro.
- After creating an AppImage you can check its SHA256 with `sha256sum Standalone/Watercan.AppImage` and the runtime dependencies for the bundled binary at `Standalone/Watercan.AppDir/usr/bin/Watercan` using `ldd`.

If you want, I can try building the AppImage here and verify it — I already generated `Standalone/Watercan.AppImage` in your workspace during the build step.